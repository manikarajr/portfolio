...
export function HeroVisual() {
  ...
  const years = useCountUp(profile.yearsExperience, isInView);
  ...
  return (
    <div ref={ref} className="relative mx-auto w-full max-w-sm lg:max-w-none">
      ...
      <div className="mt-14 flex flex-col items-center gap-4">
        <div className="flex items-center gap-3 text-xs font-semibold uppercase tracking-wide text-neutral-500 dark:text-slate-400">
        ...
        <ul className="flex items-center gap-3">
          {CORE_STACK.map((tech) => (
            <li key={tech}>
              <TechChip name={tech} variant="tile" colored />
            </li>
          ))}
        </ul>
      </div>
